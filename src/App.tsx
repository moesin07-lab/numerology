...
const profile = {
  ...
  tagline: "Master Builder with a Healer’s Heart",
  palette: {
    // Golden sunflower vibes 🌻
    primary: "#F4C847",
    primaryText: "#1a1a1a",
    accent: "#0F172A",
    soft: "#FFF7DF",
...
const core = {
  lifePath: "22/4 – Master Builder",
  expression: "7 – The Seeker / Analyst",
  soulUrge: "6 – The Nurturer / Healer",
  personality: "1 – The Pioneer",
  birthday: "7 – Intuitive Thinker",
  personalYear: {
    year: 2025,
    number: 9,
...
const strengths = [
  {
    ...
    text:
      "22/4 blends big-picture vision with practical systems. You’re built to design organizations, programs, and movements that actually work.",
  },
  {
    icon: Brain,
  ...
  },
  {
    ...
    text:
      "Personality 1 leads with originality and courage. You initiate—others follow.",
  },
];
...
const challenges = [
  {
    ...
    text:
      "22/4 can carry heavy responsibility. Progress beats perfection—ship, iterate, refine.",
  },
  {
    icon: Flower2,
  ...
  {
    icon: Compass,
    title: "Solitude → Isolation",
    text:
      "7 needs retreats to recharge, but don’t disappear. Share the inner world you polish in private.",
  },
];
...
const loveNotes = [
  "Your love isn’t meant to be proven by struggle. Choose reciprocity over potential.",
  "Detachment with compassion: help without carrying.",
  "Share the heart you protect—your truth attracts equals.",
];
  ...
  return (
    ...
    >
      ...
      <section className="mx-auto max-w-6xl px-4 pb-8 pt-10">
          ...
          <div className="grid gap-6 md:grid-cols-2">
            <div>
              ...
              <p className="mt-3 max-w-prose text-slate-700">
                A clean, modern snapshot of your core codes and the path they outline—rooted in your Life Path 22/4 and guided by your 7–6–1 harmony.
              </p>
            ...
            <Card className="border-black/10 bg-white/80 shadow-sm backdrop-blur">
              <CardHeader>
                ...
                <CardDescription>Personal Year {core.personalYear.year}: {core.personalYear.number} — {core.personalYear.theme}</CardDescription>
              </CardHeader>
              <CardContent className="text-sm text-slate-700">
                This is your spiritual editing phase. Release what’s complete to clear the runway for your Year 1 rebirth in 2026.
              </CardContent>
              <CardFooter>
        ...
        <Marquee items={[
          "Master Builder 22 → Vision into Structure",
          "Seeker 7 → Truth, Research, Intuition",
          "Heart 6 → Care, Harmony, Service",
          "Pioneer 1 → Courage, Initiative",
          "2026 Year 1 → Rebirth",
        ]} />
      </section>
      ...
      <section className="mx-auto max-w-6xl px-4 pb-24 pt-4">
        <Tabs defaultValue="overview">
          <TabsList className="flex w-full flex-wrap gap-2 rounded-xl border border-black/5 bg-white/70 p-1 backdrop-blur">
            ...
            <TabsTrigger value="roadmap">2025–2030</TabsTrigger>
          ...
          <TabsContent value="overview" className="mt-6">
            <div className="grid gap-4 md:grid-cols-3">
              <Card className="border-amber-200/60 bg-white/80 shadow-sm backdrop-blur">
                <CardHeader>
                  <SectionTitle icon={Sparkles} title="Essence" subtitle="Wisdom → Systems that serve" />
                </CardHeader>
                <CardContent className="text-sm text-slate-700">
                  You’re here to turn insight into institutions: programs, platforms, and communities built with heart and rigor.
                </CardContent>
              </Card>
              <Card className="border-emerald-200/60 bg-white/80 shadow-sm backdrop-blur">
                <CardHeader>
                  <SectionTitle icon={Star} title="Superpowers" subtitle="See patterns • Build pathways" />
                </CardHeader>
                <CardContent className="text-sm text-slate-700">
                ...
                <Card key={r.year} className="border-black/10 bg-white/80 shadow-sm backdrop-blur">
                  <CardHeader>
                    <div className="flex items-center justify-between">
                      <SectionTitle icon={CalendarDays} title={`${r.year} — Personal Year ${r.number}`} subtitle={r.label} />
                  ...
                  <CardContent>
                    <ul className="grid gap-2 md:grid-cols-2">
                      {r.notes.map((n, i) => (
                        <li key={i} className="text-sm text-slate-700">• {n}</li>
                      ))}
                    </ul>
                  </CardContent>
                </Card>
            ...
            <div className="grid gap-4 md:grid-cols-2">
              {pinnacles.map((p, i) => (
                <Card key={i} className="border-black/10 bg-white/80 shadow-sm backdrop-blur">
                  <CardHeader>
                    <div className="flex items-center justify-between">
                      <SectionTitle icon={Gem} title={`Pinnacle ${i + 1}: ${p.range}`} subtitle={`Number ${p.number} • Challenge ${p.challenge}`} />
          ...
          <TabsContent value="love" className="mt-6">
            <Card className="border-black/10 bg-white/80 shadow-sm backdrop-blur">
              <CardHeader>
                <SectionTitle icon={Heart} title="Love Blueprint" subtitle="Reciprocity • Safety • Shared Destiny" />
              </CardHeader>
              <CardContent className="grid gap-6 md:grid-cols-3">
                <div>
                  <h4 className="font-semibold">Your Pattern</h4>
                  <p className="mt-2 text-sm text-slate-700">Give deeply, then withdraw when it isn’t matched. New rule: energy must be mutual—early.</p>
                </div>
                <div>
      ...
      <footer className="border-t border-black/5 bg-white/60 py-8 text-center text-sm text-slate-600">
        Built with love and logic • © {new Date().getFullYear()} {profile.name}
      </footer>
    </div>
  );
}
